...
- **FFmpeg 9.0, 8.1.1 & 8.0.1** — three prebuilt versions provided, pick whichever you need
...
All versions share the same layout, here we use `ffmpeg-9.0/` as an example:
...
├── libffmpeg.so         # Merged shared library — all 7 modules in one .so (76 MB, not stripped)
├── include/
│   ├── libavcodec/
│   ├── libavdevice/
│   ├── libavfilter/
...
> Older builds live in `ffmpeg-8.1.1/` and `ffmpeg-8.0.1/` with nearly identical file sizes.
...
| Item | Value |
|---|---|
| FFmpeg Version | 9.0, 8.1.1, 8.0.1 |
...
- **This build**: tag `n9.0`, `n8.1.1`, `n8.0.1`
